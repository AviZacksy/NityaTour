/** Default fleet folder names under `public/` — each folder contains `data.json`. */
export const DEFAULT_FLEET_FOLDERS: string[] = [
  "Ertiga model 2024",
...
/** Fallback image filenames when a car's `data.json` omits `images`. */
...
export function resolveFleetFolders(fromConfig?: string[] | null): string[] {
  if (Array.isArray(fromConfig) && fromConfig.length > 0) {
    return fromConfig.filter((f) => typeof f === "string" && f.trim().length > 0);
  }
  return [...DEFAULT_FLEET_FOLDERS];
}
