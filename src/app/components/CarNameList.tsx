...
const CarNameList: React.FC<{ cars: Car[] }> = ({ cars }) => {
  ...
  return (
    <div className="mx-auto mt-10 max-w-md rounded-none border border-stone-200 bg-[var(--surface)] shadow-sm">
      <button
        type="button"
      ...
      >
        <ul className="max-h-[28rem] space-y-2 overflow-y-auto px-4 py-4">
          {cars.map((car, idx) => (
            <li
              key={idx}
              className="rounded-none border border-stone-100 bg-stone-50 px-4 py-2.5 text-center text-sm font-medium text-stone-800"
            >
              {car.model}
            </li>
          ))}
